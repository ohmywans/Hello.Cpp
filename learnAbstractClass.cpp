...
1. 클래스를 선언하는 부분
2. 클래스 메소드를 정의하는 부분           */
// 다음은 예시 <주제 : 주식, Stock> 자세한 설명은 클래스 코드 마무리 후 <*설명*> 참조
#include <iostream>
...
// 생성자와 파괴자가 주석처리 되어도 코드는 정상적으로 동작한다. 관련 설명은 Line 138부터 참조

/* <*설명*> / 이렇게 클래스를 만드는거야~
private, public : 이 클래스 멤버에 대한 접근을 제한하는 역할
가령, Stock이라는 클래스는 현재 name, shares, share_val, total_val을 멤버로써 이들은 모두 private의
멤버들이다. Stock 클래스가 가진 private 함수 멤버들의 값을 변경하려면 반드시 public에 선언되어 있는 
함수들을 통해서 값을 변경해줘야 한다. 예를 들어, float share_val을 변경한다고 할 때 public 함수에 정의
되어 있는 update를 사용해야 한다. 그러므로, public에 정의되어 있는 멤버 함수들은 프로그램이 private에 
정의된 멤버 변수들의 값을 변경해주거나 변경할 수 있는 다리 역할을 해준다. 프로그램이 private함수에 직접
적으로 접근할 수 없고 public 함수를 통해야만 가능하기 때문에 이렇게 데이터에 직접 접근하지 못하게 차단
하는 것을 '데이터 은닉' 이라고 한다. 이것은 추상화의 여러 예 중 하나이다.
정리.
